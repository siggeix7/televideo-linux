...
class StaticAssetTests(SimpleTestCase):
    ...
    def test_news_search_highlights_matches(self):
        app_js = Path(__file__).resolve().parents[1] / "static" / "news" / "app.js"
        source = app_js.read_text(encoding="utf-8")

        self.assertIn("function appendHighlightedText", source)
        self.assertIn("search-hit", source)

    def test_news_scroll_respects_reduced_motion(self):
        app_js = Path(__file__).resolve().parents[1] / "static" / "news" / "app.js"
        source = app_js.read_text(encoding="utf-8")

        self.assertIn("prefers-reduced-motion: reduce", source)
        self.assertIn('behavior: reduceMotion && reduceMotion.matches ? "auto" : "smooth"', source)
