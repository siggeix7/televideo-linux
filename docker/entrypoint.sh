...
# When OpenWeather is off, meteo comes from sections — use meteo interval.
if [ -n "${OPENWEATHER_API_KEY:-}" ]; then
    SECTIONS_INTERVAL="${TELETEXT_SECTION_REFRESH_SECONDS:-21600}"
else
    SECTIONS_INTERVAL="${METEO_SECTION_REFRESH_SECONDS:-3600}"
fi
python web/manage.py fetch_sections --loop --interval "${SECTIONS_INTERVAL}" &

# Optional OpenWeatherMap fallback for missing province-capital weather.
if [ -n "${OPENWEATHER_API_KEY:-}" ]; then
    python web/manage.py refresh_openweather --loop --interval "${METEO_SECTION_REFRESH_SECONDS:-3600}" &
fi
